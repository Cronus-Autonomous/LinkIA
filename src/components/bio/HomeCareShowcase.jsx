import React, { useState, useRef } from 'react';
import { 
  ShoppingBag, 
  RotateCw, 
  ChevronLeft, 
  ChevronRight, 
  Sparkles, 
  CheckCircle2, 
  Flame, 
  PackageCheck 
} from 'lucide-react';

// Categorias/Procedimentos para o mini-carrossel de filtros
const PROCEDURES = [
  { id: 'all', label: 'Todos' },
  { id: 'skincare', label: 'Skincare Diário' },
  { id: 'peeling', label: 'Peeling Químico' },
  { id: 'botox', label: 'Toxina Botulínica' },
  { id: 'pre-procedimento', label: 'Pré-Procedimento' },
  { id: 'pos-cirurgico', label: 'Pós-Cirúrgico' },
];

// Dados simulados dos produtos Home Care
const PRODUCTS = [
  {
    id: 'prod-1',
    name: 'Sérum Renovador Floral',
    brand: 'Avoskin Bio',
    price: 159.90,
    originalPrice: 189.90,
    tag: 'MAIS VENDIDO',
    tagType: 'bestseller',
    stock: 4,
    bgColor: 'bg-purple-100/70 border-purple-200',
    image: 'https://images.unsplash.com/photo-1620916566398-39f1143ab7be?q=80&w=600&auto=format&fit=crop',
    procedures: ['skincare', 'peeling'],
    description: 'Sérum concentrado com extratos florais e ácido hialurônico para regeneração celular e brilho natural.',
    indications: ['Pós-Peeling Químico', 'Pele Opaca ou Desidratada', 'Uso Diário Noturno']
  },
  {
    id: 'prod-2',
    name: 'Creme Reparador Ceramidas',
    brand: 'Leneige Derm',
    price: 189.00,
    originalPrice: 220.00,
    tag: '15% OFF',
    tagType: 'discount',
    stock: 2,
    bgColor: 'bg-blue-100/70 border-blue-200',
    image: 'https://images.unsplash.com/photo-1608248597261-833244675b16?q=80&w=600&auto=format&fit=crop',
    procedures: ['peeling', 'pos-cirurgico', 'skincare'],
    description: 'Restaura a barreira cutânea e acalma a pele sensibilizada por procedimentos invasivos ou queimaduras solares.',
    indications: ['Recuperação Pós-Laser/Peeling', 'Peles Sensíveis', 'Hidratação Profunda 24h']
  },
  {
    id: 'prod-3',
    name: 'Espuma Suave Cleanser Pro',
    brand: 'SkinCare Lab',
    price: 98.90,
    originalPrice: null,
    tag: 'NOVIDADE',
    tagType: 'new',
    stock: 8,
    bgColor: 'bg-pink-100/70 border-pink-200',
    image: 'https://images.unsplash.com/photo-1556228720-195a672e8a03?q=80&w=600&auto=format&fit=crop',
    procedures: ['botox', 'pre-procedimento', 'skincare'],
    description: 'Higienização profunda sem agredir o PH fisiológico da pele. Prepara o tecido para absorver os ativos pós-procedimento.',
    indications: ['Higienização Pré-Injetáveis', 'Remoção de Maquiagem', 'Limpeza Diária']
  },
  {
    id: 'prod-4',
    name: 'Protetor Solar FPS 70 com Cor',
    brand: 'DermoShield',
    price: 135.00,
    originalPrice: 150.00,
    tag: 'ESSENCIAL',
    tagType: 'bestseller',
    stock: 3,
    bgColor: 'bg-amber-100/70 border-amber-200',
    image: 'https://images.unsplash.com/photo-1598440947619-2c35fc9aa908?q=80&w=600&auto=format&fit=crop',
    procedures: ['peeling', 'botox', 'skincare', 'pos-cirurgico'],
    description: 'Proteção contra luz azul e raios UVA/UVB com toque seco e cobertura natural de imperfeições.',
    indications: ['Prevenção de Manchas Pós-Procedimento', 'Uso Diário Ocultador de Vermelhidão']
  }
];

// Sub-componente do Card do Produto com rotação 3D (Flip Effect)
function ProductFlipCard({ product, onReserve }) {
  const [isFlipped, setIsFlipped] = useState(false);

  return (
    <div className="snap-start flex-shrink-0 w-[270px] sm:w-[300px] h-[390px] [perspective:1000px] group">
      {/* Container 3D com rotação controlada */}
      <div
        className={`relative w-full h-full duration-700 [transform-style:preserve-3d] transition-transform ${
          isFlipped ? '[transform:rotateY(180deg)]' : ''
        }`}
      >
        {/* ================= FACE FRONTAL ================= */}
        <div
          className={`absolute inset-0 w-full h-full rounded-3xl border p-4 flex flex-col justify-between [backface-visibility:hidden] shadow-sm bg-white overflow-hidden`}
        >
          {/* Topo do Card: Badges e Botão de Girar */}
          <div className="flex items-center justify-between z-10">
            {/* Tag / Micro Pill */}
            {product.tag && (
              <span
                className={`text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider flex items-center gap-1 ${
                  product.tagType === 'bestseller'
                    ? 'bg-amber-400 text-zinc-900'
                    : product.tagType === 'discount'
                    ? 'bg-rose-500 text-white'
                    : 'bg-zinc-800 text-white'
                }`}
              >
                {product.tagType === 'bestseller' && <Flame className="w-3 h-3" />}
                {product.tag}
              </span>
            )}

            {/* Botão de Virar a Carta */}
            <button
              onClick={() => setIsFlipped(true)}
              className="p-2 rounded-full bg-white/80 backdrop-blur-md border border-zinc-200 text-zinc-600 hover:text-zinc-900 hover:bg-zinc-100 transition-colors shadow-xs ml-auto cursor-pointer"
              title="Ver detalhes do produto"
              aria-label="Virar card"
            >
              <RotateCw className="w-4 h-4" />
            </button>
          </div>

          {/* Área da Imagem do Produto */}
          <div className="relative flex-1 flex items-center justify-center my-2 group-hover:scale-105 transition-transform duration-300">
            {/* Fundo colorido arredondado estilizado (Inspirado no design do filtro/cards) */}
            <div className={`absolute inset-3 rounded-2xl ${product.bgColor} opacity-80`} />
            <img
              src={product.image}
              alt={product.name}
              className="relative z-10 max-h-[190px] w-auto object-contain drop-shadow-md"
            />
          </div>

          {/* Painel Inferior de Informações e Ação */}
          <div className="bg-zinc-50 border border-zinc-100 rounded-2xl p-3 z-10">
            <div className="flex justify-between items-start mb-1">
              <div>
                <h3 className="text-sm font-bold text-zinc-900 leading-tight">
                  {product.name}
                </h3>
                <p className="text-[11px] text-zinc-500">{product.brand}</p>
              </div>
            </div>

            {/* Preço e Micro-pill de Estoque */}
            <div className="flex items-center justify-between mt-2 pt-2 border-t border-zinc-200/60">
              <div>
                <div className="flex items-baseline gap-1.5">
                  <span className="text-base font-extrabold text-zinc-900">
                    R$ {product.price.toFixed(2).replace('.', ',')}
                  </span>
                  {product.originalPrice && (
                    <span className="text-xs text-zinc-400 line-through">
                      R$ {product.originalPrice.toFixed(2).replace('.', ',')}
                    </span>
                  )}
                </div>
                {/* Stock micro-pill */}
                <span className="text-[10px] font-medium text-emerald-600 flex items-center gap-1 mt-0.5">
                  <PackageCheck className="w-3 h-3 inline" /> Restam {product.stock} un.
                </span>
              </div>

              {/* Botão de Compra no WhatsApp */}
              <button
                onClick={() => onReserve(product)}
                className="px-3.5 py-2 rounded-xl bg-zinc-900 hover:bg-emerald-600 text-white font-semibold text-xs transition-all duration-300 flex items-center gap-1.5 shadow-sm active:scale-95 cursor-pointer"
              >
                <ShoppingBag className="w-3.5 h-3.5" />
                Reservar
              </button>
            </div>
          </div>
        </div>

        {/* ================= FACE TRASEIRA (VERSO) ================= */}
        <div
          className="absolute inset-0 w-full h-full rounded-3xl border border-zinc-200 p-5 flex flex-col justify-between [backface-visibility:hidden] [transform:rotateY(180deg)] shadow-md bg-zinc-900 text-white"
        >
          {/* Header do Verso */}
          <div>
            <div className="flex items-center justify-between mb-3">
              <span className="text-[10px] font-bold text-amber-400 uppercase tracking-widest flex items-center gap-1">
                <Sparkles className="w-3 h-3" /> Ficha Técnica
              </span>
              <button
                onClick={() => setIsFlipped(false)}
                className="p-1.5 rounded-full bg-zinc-800 text-zinc-400 hover:text-white transition-colors cursor-pointer"
                aria-label="Voltar para a frente"
              >
                <RotateCw className="w-4 h-4" />
              </button>
            </div>

            <h4 className="text-base font-bold text-white mb-1">{product.name}</h4>
            <p className="text-xs text-zinc-300 leading-relaxed mb-4">
              {product.description}
            </p>

            {/* Indicações do Produto */}
            <div className="space-y-1.5">
              <p className="text-[11px] font-semibold text-zinc-400 uppercase">Indicado para:</p>
              {product.indications.map((ind, idx) => (
                <div key={idx} className="flex items-start gap-1.5 text-xs text-zinc-200">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0 mt-0.5" />
                  <span>{ind}</span>
                </div>
              ))}
            </div>
          </div>

          {/* Rodapé do Verso com o mesmo CTA de Conversão */}
          <div className="pt-3 border-t border-zinc-800 flex items-center justify-between">
            <div>
              <p className="text-[10px] text-zinc-400">Valor com desconto:</p>
              <p className="text-sm font-bold text-amber-400">
                R$ {product.price.toFixed(2).replace('.', ',')}
              </p>
            </div>
            <button
              onClick={() => onReserve(product)}
              className="px-4 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-zinc-950 font-bold text-xs transition-colors flex items-center gap-1.5 cursor-pointer"
            >
              <ShoppingBag className="w-3.5 h-3.5" />
              Garantir no Whats
            </button>
          </div>
        </div>
      </div>
    </div>
  );
}

// COMPONENTE PRINCIPAL
export default function HomeCareShowcase({ phoneNumber = '5511999999999' }) {
  const [selectedCategory, setSelectedCategory] = useState('all');
  const productsScrollRef = useRef(null);
  const filterScrollRef = useRef(null);

  // Filtragem dos produtos de acordo com a categoria selecionada
  const filteredProducts = selectedCategory === 'all'
    ? PRODUCTS
    : PRODUCTS.filter(p => p.procedures.includes(selectedCategory));

  // Ação de reserva pelo WhatsApp
  const handleReserve = (product) => {
    const text = `Olá! Gostaria de reservar o produto *${product.name}* (R$ ${product.price.toFixed(2)}) listado no Home Care do LinkIA. Há disponibilidade na clínica?`;
    window.open(`https://wa.me/${phoneNumber}?text=${encodeURIComponent(text)}`, '_blank');
  };

  // Navegação manual por setas no carrossel de produtos
  const scrollProducts = (direction) => {
    if (productsScrollRef.current) {
      const { scrollLeft, clientWidth } = productsScrollRef.current;
      const scrollAmount = clientWidth * 0.75;
      productsScrollRef.current.scrollTo({
        left: direction === 'left' ? scrollLeft - scrollAmount : scrollLeft + scrollAmount,
        behavior: 'smooth'
      });
    }
  };

  return (
    <section className="py-8 w-full max-w-4xl mx-auto px-4">
      {/* Título da Seção */}
      <div className="mb-5 text-center sm:text-left">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-100 text-purple-800 text-xs font-semibold mb-2">
          <Sparkles className="w-3.5 h-3.5" /> Manutenção & Cuidados
        </div>
        <h2 className="text-2xl sm:text-3xl font-extrabold text-zinc-900 tracking-tight">
          Vitrine Home Care
        </h2>
        <p className="text-xs sm:text-sm text-zinc-500 mt-1">
          Produtos recomendados para potencializar os resultados do seu procedimento.
        </p>
      </div>

      {/* 1. Mini Carrossel de Filtros por Procedimento */}
      <div className="relative mb-6">
        <div
          ref={filterScrollRef}
          className="flex gap-2 overflow-x-auto scrollbar-none py-1 scroll-smooth"
          style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
        >
          {PROCEDURES.map((proc) => {
            const isActive = selectedCategory === proc.id;
            return (
              <button
                key={proc.id}
                onClick={() => setSelectedCategory(proc.id)}
                className={`snap-start flex-shrink-0 px-4 py-2 rounded-2xl text-xs sm:text-sm font-semibold transition-all duration-300 cursor-pointer ${
                  isActive
                    ? 'bg-purple-600 text-white shadow-md shadow-purple-500/20 scale-105'
                    : 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200/70'
                }`}
              >
                {proc.label}
              </button>
            );
          })}
        </div>
      </div>

      {/* 2. Carrossel de Cards com Rotação (Flip Cards) */}
      <div className="relative group">
        <div
          ref={productsScrollRef}
          className="flex gap-5 overflow-x-auto snap-x snap-mandatory scrollbar-none py-2 px-1 scroll-smooth"
          style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
        >
          {filteredProducts.length > 0 ? (
            filteredProducts.map((product) => (
              <ProductFlipCard
                key={product.id}
                product={product}
                onReserve={handleReserve}
              />
            ))
          ) : (
            <div className="w-full py-12 text-center text-zinc-400 bg-zinc-50 rounded-3xl border border-dashed border-zinc-200">
              Nenhum produto cadastrado para este procedimento específico.
            </div>
          )}
        </div>

        {/* Setas de navegação desktop */}
        {filteredProducts.length > 1 && (
          <>
            <button
              onClick={() => scrollProducts('left')}
              className="absolute -left-3 top-1/2 -translate-y-1/2 z-20 w-9 h-9 rounded-full bg-white/90 border border-zinc-200 text-zinc-800 flex items-center justify-center shadow-md hover:bg-white transition-all opacity-0 group-hover:opacity-100 cursor-pointer"
              aria-label="Anterior"
            >
              <ChevronLeft className="w-5 h-5" />
            </button>
            <button
              onClick={() => scrollProducts('right')}
              className="absolute -right-3 top-1/2 -translate-y-1/2 z-20 w-9 h-9 rounded-full bg-white/90 border border-zinc-200 text-zinc-800 flex items-center justify-center shadow-md hover:bg-white transition-all opacity-0 group-hover:opacity-100 cursor-pointer"
              aria-label="Próximo"
            >
              <ChevronRight className="w-5 h-5" />
            </button>
          </>
        )}
      </div>
    </section>
  );
}